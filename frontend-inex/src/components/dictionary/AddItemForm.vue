...
// 1. Pobrane zweryfikowane ID profilu
const { verifiedActiveProfileId, isProfileStoreReady } = storeToRefs(profileStore);
// 2. Pobrane listy słowników używające bezpiecznych getterów (które muszą być zdefiniowane w Store)
const { activeCategories } = storeToRefs(categoryStore); // Zakładamy, że to bezpieczny getter
const { activeLabels } = storeToRefs(labelStore); // Zakładamy, że to bezpieczny getter
...
watchEffect(() => {
  // Używamy bezpiecznej listy activeCategories
  if (activeCategories.value.length === 1 && selectedCategoryId.value === null) {
    const singleCategory = activeCategories.value[0];
...
const handleSubmit = async () => {
  // Walidacja: Używamy activeProfileId, który jest teraz aliasem dla verifiedActiveProfileId
  if (!newItemName.value.trim() || !selectedCategoryId.value || activeProfileId.value === null) {
    message.value = {
      text: 'Uzupełnij nazwę pozycji i kategorię, oraz upewnij się, że masz aktywny profil.',
      type: 'error',
    };
    return;
  }
  ...
  message.value = { text: '', type: null }; // Używamy zweryfikowanego ID w danych do wysłania

  const newItemData: NewItemData = {
    itemName: newItemName.value.trim(),
    profileId: activeProfileId.value, // Zapewniamy, że ID jest typu number, bo walidacja to gwarantuje
  ...
  try {
    ...
    // ZMIANA: Wywołanie fetchItems powinno nastąpić tylko, gdy activeProfileId jest dostępny
...
const toggleLabel = (id: number) => {
  const index = selectedLabelIds.value.indexOf(id); // Upewniamy się, że nie dodajemy duplikatów (Twój backend przyjmuje listę)
  if (index === -1) {
...
<template>
  <div class="form-container border border-dashed border-red-300">
    ...
    <!-- Warunek używa activeProfileId, który jest teraz zweryfikowany -->
    <div
      v-if="activeProfileId === null || !isProfileStoreReady"
      class="text-center p-4 bg-yellow-50 text-yellow-700 rounded-md"
    >
      Oczekiwanie na aktywny profil...
    </div>
    <div
      v-else-if="activeProfileId === undefined"
      class="text-center p-4 bg-yellow-50 text-yellow-700 rounded-md"
    >
    ...
    <form v-else @submit.prevent="handleSubmit">
      ...
      <div class="mb-3">
        ...
        <!-- Używamy activeCategories (bezpieczny getter) -->
        <select
          id="categoryId"
        ...
        <div
          v-if="activeCategories.length === 0 && !categoryStore.isLoading"
      ...
      <div class="mb-4">
        ...
        <div class="flex flex-wrap gap-2">
          <!-- Używamy activeLabels (bezpieczny getter) -->
          <button
            type="button"
          ...
          <div
            v-if="activeLabels.length === 0 && !labelStore.isLoading"
      ...
      <button
        type="submit"
        :disabled="isSubmitting || activeProfileId === null || !selectedCategoryId"
        class="btn-primary"
        :class="{
          'btn-disabled': isSubmitting || activeProfileId === null || !selectedCategoryId,
        }"
      >
