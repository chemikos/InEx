...
// ZMIANA 1: Używamy zweryfikowanego ID profilu, aby upewnić się, że jest ono bezpieczne
...
// ZMIANA 2: activeProfileId teraz pochodzi z verifiedActiveProfileId
const activeProfileId = computed(() => verifiedActiveProfileId.value);

const handleSubmit = async () => {
  // ZMIANA 3: Walidacja na podstawie zweryfikowanego ID
  if (!newCategoryName.value.trim() || activeProfileId.value === null) {
    message.value = {
      text: 'Musisz wybrać aktywny i zweryfikowany profil oraz podać nazwę kategorii.',
      type: 'error',
    };
    return;
  }
  ...
  const newCategoryData: NewCategoryData = {
    categoryName: newCategoryName.value.trim(), // ZMIANA 4: Wysyłamy zweryfikowane ID
  ...
  try {
    ...
    message.value = {
      text:
        response.message ||
        (response.category
          ? `Dodano kategorię: ${response.category.name} (ID: ${response.category.id_category})`
          : 'Dodano kategorię.'),
      type: 'success',
    };
